...
async def create_redis_queue_pool() -> None:
    queue.pool = await create_pool(
        RedisSettings(
            ...
            password=settings.REDIS_QUEUE_PASSWORD_OR_NONE,
        )
    )
...
# -------------- mongodb --------------
async def create_mongo_client() -> None:
...
# -------------- kafka --------------
async def create_kafka_producer() -> None:
    # Kafka may not be ready when the app starts in docker-compose.
...
def lifespan_factory(
    ...
    async def lifespan(app: FastAPI) -> AsyncGenerator:
        ...
        try:
            ...
            if isinstance(settings, MongoDBSettings):
...
def create_application(
    ...
    **kwargs: Any,
    ...
    settings
        ...
        - KafkaSettings: Sets up a Kafka producer on startup and closes it on shutdown.
        - MongoDBSettings: Sets up a MongoDB client on startup and closes it on shutdown.
        ...
        - APISettings: Adds middleware for API key validation on /api routes.
